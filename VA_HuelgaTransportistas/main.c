...
void Asignacion(imatriz2d m, ivector v, ivector mejor, int n, int paso, int *mejorCoste) {
    int costeActual;
    if (paso == n) {
    ...
    for (int i = paso; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            ...
            if (costeActual < RAND_MAX && costeActual < *mejorCoste) {
                Asignacion(m, v, mejor, n, i+1, mejorCoste);
            }
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    Asignacion(tabla, v, mejor, TAM, 0, &coste);
